...
if (!PHBPermissoes::Check("logs_raros")) {
	header("Location: /index");
	die();
}
?>
...
<head>
	...
	<title><?= $config['NomeHotel']; ?> - <?= $lingua['logs-raros-titulo']; ?></title>
					...
					<div class="row row-sm">
						<div class="col-lg-12 mt-xl-4">
							<div class="card custom-card overflow-hidden">
								<div class="card-body">
									<div>
										<h6 class="main-content-label mb-1"><?= $lingua['logs-raros-texto1']; ?></h6>
										<p class="text-muted card-sub-title"><?= $lingua['logs-raros-texto2']; ?></p>
									</div>
									<div class="table-responsive">
										<table class="table" id="example1">
											<thead>
												<tr>
													<th class="wd-5p"></th>
													<th class="wd-25p"><?= $lingua['logs-raros-comprador']; ?></th>
													<th class="wd-25p"><?= $lingua['logs-raros-dono']; ?></th>
													<th class="wd-25p"><?= $lingua['logs-raros-raro']; ?></th>
													<th class="wd-15p"><?= $lingua['logs-raros-lote']; ?></th>
													<th class="wd-15p"><?= $lingua['logs-raros-quarto']; ?></th>
													<th class="wd-20p"><?= $lingua['logs-raros-data']; ?></th>
												</tr>
											</thead>
											<tbody>
												<?php
												$obtemLogs = $db->prepare("SELECT catalog_items_limited.timestamp, catalog_items_limited.item_id, users.username AS comprador, items.limited_data, items_base.item_name, items_base.public_name, items.room_id, novodono.username FROM catalog_items_limited, users, items, items_base, users novodono WHERE items_base.id = items.item_id AND items.id = catalog_items_limited.item_id AND users.id = catalog_items_limited.user_id AND novodono.id = items.user_id ORDER BY catalog_items_limited.timestamp DESC");
												$obtemLogs->execute();
												...
												?>
													<tr>
														...
														<td class="text-primary"><a target="_blank" href="<?= str_replace("%id%", $log['room_id'], $config['PathIconesMobis']); ?>"><?= $log['room_id']; ?></a></td>
