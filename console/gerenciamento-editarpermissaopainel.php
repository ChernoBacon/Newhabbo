...
if (!PHBPermissoes::Check("gerenciamento_permissoespainel_editar")) {
	header("Location: /index");
	die();
}
if (!isset($_GET['id'])) {
...
<head>
	...
	<title><?= $config['NomeHotel']; ?> - <?= $lingua['gerenciamento-editarpermissaopainel-titulo']; ?></title>
					...
					<div class="row row-sm mt-xl-4">
						<div class="col-lg-12 col-md-12">
							<div class="card custom-card">
								<div class="card-body">
									<div>
										<h6 class="main-content-label mb-1"><?= $lingua['gerenciamento-editarpermissaopainel-texto1']; ?></h6>
										<p class="text-muted card-sub-title"><?= $lingua['gerenciamento-editarpermissaopainel-texto2']; ?></p>
									</div>
									<form method="post">
										<?php
										$id = $_GET['id'];
										if (isset($_POST['salvar']) && PHBPermissoes::Check("gerenciamento_permissoespainel_editar")) {
											$count = 0;
											$sql = "UPDATE phb_hk_permissoes SET ";
											...
											try {
												...
												<strong>' . $lingua['generico-erro'] . '</strong> ' . $lingua['gerenciamento-editarpermissaopainel-errosql'] . ' (' . $e . ').</div>';
											}
											PHBLogger::AddLog($lingua['gerenciamento-editarpermissaopainel-log'] . ' ' . $id . '.');
											echo '<div class="alert alert-solid-success" role="alert">
												<button aria-label="Close" class="close" data-dismiss="alert" type="button">
												<span aria-hidden="true">&times;</span></button>
												<strong>' . $lingua['generico-sucesso'] . '</strong> ' . $lingua['gerenciamento-editarpermissaopainel-sucesso'] . '</div>';
										}
										?>
										<div class="row row-sm">
											...
											function pegaValor($campo)
											{
												global $id;
												global $db;
												$selectRanks = $db->prepare("SELECT " . str_replace(".", "_", $campo) . " FROM `phb_hk_permissoes` WHERE id = :id;");
												$selectRanks->bindParam(':id', $id);
												$selectRanks->execute();
												return $selectRanks->fetch()[0];
											}
											$selectRanks = $db->prepare("SELECT * FROM `information_schema`.`COLUMNS` WHERE TABLE_SCHEMA='" . $database['db'] . "' AND TABLE_NAME='phb_hk_permissoes' ORDER BY ORDINAL_POSITION;");
											$selectRanks->bindParam(':id', $id);
											$selectRanks->execute();
										...
										<button type="submit" class="btn ripple btn-primary" name="salvar" style="float: right;margin-top: 15px;"><?= $lingua['gerenciamento-editarpermissaopainel-salvar']; ?></button>
									</form>
								</div>
							</div>
						</div>
