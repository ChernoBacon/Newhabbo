...
if (!PHBPermissoes::Check("relatorio_onlines")) {
	header("Location: /index");
	die();
}
?>
...
<head>
	...
	<title><?= $config['NomeHotel']; ?> - <?= $lingua['relatorio-onlines-titulo']; ?></title>
					...
					<div class="row row-sm">
						<div class="col-lg-12 mt-xl-4">
							<div class="card custom-card overflow-hidden">
								<div class="card-body">
									<div>
										<h6 class="main-content-label mb-1"><?= $lingua['relatorio-onlines-texto1']; ?></h6>
										<p class="text-muted card-sub-title"><?= $lingua['relatorio-onlines-texto2']; ?></p>
									</div>
									<div class="table-responsive">
										<table id="exportexample" class="table table-bordered border-t0 key-buttons text-nowrap w-100">
											<thead>
												<tr>
													<th class="wd-25p"><?= $lingua['relatorio-onlines-data']; ?></th>
													<th class="wd-25p"><?= $lingua['relatorio-onlines-onlines']; ?></th>
												</tr>
											</thead>
											<tbody>
												<?php

												$obtemRelatorio = $db->prepare("SELECT *, DATE_FORMAT(FROM_UNIXTIME(`timestamp`), '%Y-%m-%d') AS data FROM graficos_phbplugin order by data desc;");
												$obtemRelatorio->execute();
												while ($fetch = $obtemRelatorio->fetch()) {

													if (!isset($RelatorioPHB[$fetch['data']]))
														$RelatorioPHB[$fetch['data']] = $fetch['onlines'];

													if ($fetch['onlines'] > $RelatorioPHB[$fetch['data']])
														$RelatorioPHB[$fetch['data']] = $fetch['onlines'];
												}
