...
if (!PHBPermissoes::Check("logs_login")) {
	header("Location: /index");
	die();
}
?>
...
<head>
	...
	<title><?= $config['NomeHotel']; ?> - <?= $lingua['logs-login-titulo'];?></title>
					...
					<div class="row row-sm">
						<div class="col-lg-12 mt-xl-4">
							<div class="card custom-card overflow-hidden">
								<div class="card-body">
									<div>
										<h6 class="main-content-label mb-1"><?= $lingua['logs-login-texto1'];?></h6>
										<p class="text-muted card-sub-title"><?= $lingua['logs-login-texto2'];?></p>
									</div>
									<div class="table-responsive">
										<table class="table" id="example1">
											<thead>
												<tr>
													<th class="wd-25p"><?= $lingua['logs-login-usuario'];?></th>
													<th class="wd-25p"><?= $lingua['logs-login-ip'];?></th>
													<th class="wd-15p"><?= $lingua['logs-login-navegador'];?></th>
													<th class="wd-15p"><?= $lingua['logs-login-data'];?></th>
												</tr>
											</thead>
											<tbody>
												<?php
												$obtemLogs = $db->prepare("SELECT user_session_log.*, users.username FROM user_session_log,users WHERE users.id = user_session_log.userid;");
												$obtemLogs->execute();
												...
												?>
													<tr>
														...
														<td class="text-primary"><?= strip_tags($log['browser']); ?></td>
