...
if (!PHBPermissoes::Check("logs_hk")) {
	header("Location: /index");
	die();
}
?>
...
<head>
	...
	<title><?= $config['NomeHotel']; ?> - <?= $lingua['logs-hk-titulo'];?></title>
					...
					<div class="row row-sm">
						<div class="col-lg-12 mt-xl-4">
							<div class="card custom-card overflow-hidden">
								<div class="card-body">
									<div>
										<h6 class="main-content-label mb-1"><?= $lingua['logs-hk-texto1'];?></h6>
										<p class="text-muted card-sub-title"><?= $lingua['logs-hk-texto2'];?></p>
									</div>
									<div class="table-responsive">
										<table class="table" id="example1">
											<thead>
												<tr>
													<th class="wd-25p"><?= $lingua['logs-hk-usuario'];?></th>
													<th class="wd-15p"><?= $lingua['logs-hk-acao'];?></th>
													<th class="wd-20p"><?= $lingua['logs-hk-data'];?></th>
												</tr>
											</thead>
											<tbody>
												<?php
												$obtemLogs = $db->prepare("SELECT phb_hk_logs.log, phb_hk_logs.data, users.username FROM phb_hk_logs, users WHERE users.id = phb_hk_logs.usuario ORDER BY phb_hk_logs.id DESC LIMIT 2500");
												$obtemLogs->execute();
												...
												?>
													<tr>
														...
														<td class="text-primary"><?= date("d/m/y H:i", $log['data']); ?></td>
													</tr>
												<?php } ?>
											</tbody>
										</table>
