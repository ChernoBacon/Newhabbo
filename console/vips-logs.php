...
if (!PHBPermissoes::Check("vips_logs")) {
	header("Location: /index");
	die();
}
?>
...
<head>
	...
	<title><?= $config['NomeHotel']; ?> - <?= $lingua['vips-logs-titulo']; ?></title>
					...
					<div class="row row-sm">
						<div class="col-lg-12 mt-xl-4">
							<div class="card custom-card overflow-hidden">
								<div class="card-body">
									<div>
										<h6 class="main-content-label mb-1"><?= $lingua['vips-logs-texto1']; ?></h6>
										<p class="text-muted card-sub-title"><?= $lingua['vips-logs-texto2']; ?></p>
									</div>
									<div class="table-responsive">
										<table class="table" id="example1">
											<thead>
												<tr>
													<th class="wd-25p"><?= $lingua['vips-logs-usuario']; ?></th>
													<th class="wd-15p"><?= $lingua['vips-logs-staff']; ?></th>
													<th class="wd-15p"><?= $lingua['vips-logs-log']; ?></th>
													<th class="wd-20p"><?= $lingua['vips-logs-data']; ?></th>
												</tr>
											</thead>
											<tbody>
												<?php
												$obtemLogs = $db->prepare("SELECT * FROM phb_hk_vips_logs ORDER BY id DESC");
												$obtemLogs->execute();
												while ($log = $obtemLogs->fetch()) {
												?>
													<tr>
														<td class="text-primary"><?= PHBTools::GetUsernameById($log['user_id']); ?></td>
														<td class="text-primary"><?= PHBTools::GetUsernameById($log['staff_id']); ?></td>
														<td class="text-primary"><?= $log['log']; ?></td>
