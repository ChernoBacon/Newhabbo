...
if (!PHBPermissoes::Check("logs_marketplace")) {
	header("Location: /index");
	die();
}
?>
...
<head>
	...
	<title><?= $config['NomeHotel']; ?> - <?= $lingua['logs-marketplace-titulo']; ?></title>
					...
					<div class="row row-sm">
						<div class="col-lg-12 mt-xl-4">
							<div class="card custom-card overflow-hidden">
								<div class="card-body">
									<div>
										<h6 class="main-content-label mb-1"><?= $lingua['logs-marketplace-texto1']; ?></h6>
										<p class="text-muted card-sub-title"><?= $lingua['logs-marketplace-texto2']; ?></p>
									</div>
									<div class="table-responsive">
										<table class="table" id="example1">
											<thead>
												<tr>
													<th class="wd-5p"></th>
													<th class="wd-25p"><?= $lingua['logs-marketplace-vendedor']; ?></th>
													<th class="wd-15p"><?= $lingua['logs-marketplace-comprador']; ?></th>
													<th class="wd-15p"><?= $lingua['logs-marketplace-mobi']; ?></th>
													<th class="wd-15p"><?= $lingua['logs-marketplace-lote']; ?></th>
													<th class="wd-20p"><?= $lingua['logs-marketplace-data']; ?></th>
												</tr>
											</thead>
											<tbody>
												<?php
												$obtemLogs = $db->prepare("SELECT phbplugin_marketplace_logs.time, items_base.item_name, items_base.public_name, vendedor.username AS vendedor, comprador.username AS comprador, items.limited_data FROM phbplugin_marketplace_logs, users comprador, users vendedor, items_base, items WHERE items.id = phbplugin_marketplace_logs.item_id AND comprador.id = phbplugin_marketplace_logs.comprador AND vendedor.id = phbplugin_marketplace_logs.vendedor AND items_base.id = phbplugin_marketplace_logs.base_item_id ORDER BY phbplugin_marketplace_logs.id DESC;");
												$obtemLogs->execute();
